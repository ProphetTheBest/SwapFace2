...
private fun UByteArray.asByteArray(): ByteArray =
    ByteArray(this.size) { this[it].toByte() }

private fun ubyteArrayOf(vararg elements: UByte): ByteArray =
    ByteArray(elements.size) { elements[it].toByte() }

// --- Overloads for Mat.get/put using only ByteArray for OpenCV interop ---
